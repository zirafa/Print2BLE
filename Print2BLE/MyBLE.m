...
{
    const char *szTypes[] = {"MTP-II", "MTP-2", "MTP-3", "MTP-3F", "PeriPage+", "PeriPage_", "GT01", "GT02", "GB01", "GB02", "YHK-54A8", "MX06", "D110-E8", "M02S" NULL};
    const uint8_t ucTypes[] = {PRINTER_MTP2, PRINTER_MTP2, PRINTER_MTP3, PRINTER_MTP3, PRINTER_PERIPAGEPLUS, PRINTER_PERIPAGE, PRINTER_CAT, PRINTER_CAT, PRINTER_CAT, PRINTER_CAT, PRINTER_PANDA, PRINTER_CAT, PRINTER_PANDA, PRINTER_PERIPAGEPLUS};
    char szTemp[32];
    ...
    while (szTypes[i] != NULL) {
        if (strcmp(szTemp, szTypes[i]) == 0) {
            ucType = ucTypes[i];
            return ucType;
        } else { // try truncating the name to match
            if (memcmp(szTemp, szTypes[i], strlen(szTypes[i])) == 0) {
                ucType = ucTypes[i];
                return ucType;
            }
        }
